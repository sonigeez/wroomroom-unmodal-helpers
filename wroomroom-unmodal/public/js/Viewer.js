...
function getRoomId() {
    ...
    console.log('wilcard entry', queryRoomId);
...
function whoAreYou() {
    ...
    Swal.fire({
        ...
        inputValidator: (roomId) => {
            if (!roomId) return 'Please enter your room id';
            roomId = filterXSS(roomId);
            if (isHtml(roomId)) return 'Invalid name!';
            if (!getCookie(room_id + '_name')) {
                window.localStorage.peer_name = roomId;
            }
            setCookie(room_id + '_roomId', roomId, 30);
            room_id = roomId;
            // peer_name = name;
        },
    }).then(() => {
...
function setSelectsInit() {
    ...
    if (initVideoSelect.value) changeCamera(0);
}
